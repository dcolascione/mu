...
#ifdef HAVE_LIBURING
struct IoUringState {
	static constexpr size_t max_ring_batch_size{16384};
	...
	IoUringState() {
		int ret;
		if (const auto disable_io_uring = getenv("MU_DISABLE_IO_URING");
		    disable_io_uring && !strcmp(disable_io_uring, "1")) {
			ret = -ENOSYS;
		} else {
			ret = io_uring_queue_init(max_ring_batch_size, &uring,
...
try_bulk_stat_io_uring(const std::vector<dentry_t>& entries,
		       ...
		       F&& fn)
{
	// SINGLE_ISSUER rings must be submitted by one thread only.
	static thread_local IoUringState ring;
	...
	const size_t max_batch_size = std::min(IoUringState::max_ring_batch_size, n_entries);
	...
	while (n_processed < n_entries) {
		...
		if (n_to_await > 0) {
			int ret = io_uring_submit(&ring.uring);
			if (ret < 0) {
		...
		for (size_t i = 0; i < n_to_await; ++i) {
			...
			io_uring_cqe_seen(&ring.uring, cqe);
			fn(&dentries[dentry_idx], stat_buf);
		}
...
Scanner::Private::process_dir(const std::string& path, bool is_maildir)
{
	...
	if (is_maildir && !maildirs_only_mode() &&
	    try_bulk_stat_io_uring(dir_entries, ::dirfd(dir),
				   maildirs_only_mode(),
